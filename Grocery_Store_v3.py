...
# Isaac Schultz
# April 27, 2018
# Sitka Tech Application Engineering Exercise
...
class Grocery_options(object):
    ...
    def get_contents(self):
        all_items = []
        #Display cart sorted alphabetically
...
def checkout(cart):
    cost = cart.get_total()
    print("\nWe hope you found everything you were looking for, how would you like to pay today?\n1. Cash\n2. Credit\n3. Bitcoin\n\nPlease enter an option number to continue")
    choice2 = input()
    if choice2 == "1":
        #Cash payment option
        ...
        response = input()
        print("\nGreat answer, I'll pass it along to Isaac for you. Thanks again for shopping with us and we hope to see you again soon!\n")
        return True
        ...
        try:
            ...
            btc_price = float(response.json()["last"])
            btc_total = cost / btc_price
        except:
            print("\nUnable to get bitcoin price at the moment. Please try again later.")
            return False
        print("\nThe current BTC price is: $", format(btc_price, ".2f"), " Your total is: ", format(btc_total, ".10f"), " BTC. Please send this amount to 351JZPQv5r4LJNSQeQWnhwABfiE6vuw8m2. Thank you and have a nice day.\n", sep = '')
        return True
...
if __name__ == "__main__":
    ...
    apples = Grocery_product("Apples", "lb.", 1.5, 0)
    ...
    while not done_shopping:
        ...
        if choice == "1":
            ...
            if len(cart.contents) == 0:
                print("Your cart is empty, you can add items by pressing 3")
            else:
            ...
            if item:
                #Check to see if item is available, then add desired quantity
                ...
                cart.add_item(item)
                print("\nYou added ", item.name, " to your cart\n", sep = '')
            else:
            ...
            if cart.item_exists(item_name):
                ...
                print("\nSorry, but acording to my records that item is not in your cart\n")
            ...
            if store_price >= suggestion_price:
                # Messages on the  price found versus what was requested
